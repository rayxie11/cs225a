...
int main() {
	...
	*/
	...
	// M when th1=0, d2=0.5, -pi/2<=th3<=pi/2
	cout << " " << endl;
	cout << "printing m11, m22, m33 for 2ei)" << endl;
	double step = M_PI/100;
	double th3 = -1.0*M_PI/2;
	for (int i = 0; i <= 100; i ++){
		robot->_q << 0.0, 0.5, th3;
		robot->updateModel();
		cout << th3 << " " << robot->_M.coeff(0,0) << " " << robot->_M.coeff(1,1) << " " << robot->_M.coeff(2,2) << endl;
		th3 += step;
	}
	
	// M when th1=0, th3=0, 0<=d2<=2
	cout << " " << endl;
	cout << "printing m11, m22, m33 for 2eii)" << endl;
	double step1 = 2.0/100;
	double d2 = 0;
	for (int i = 0; i <= 100; i ++){
		robot->_q << 0.0, d2, 0.0;
		robot->updateModel();
		cout << d2 << " " << robot->_M.coeff(0,0) << " " << robot->_M.coeff(1,1) << " " << robot->_M.coeff(2,2) << endl;
		d2 += step1;
	}
	
	// G when th1=0, d2=0.5, -pi/2<=th3<=pi/2
	cout << " " << endl;
	cout << "printing g1, g2, g3 for 2fi)" << endl;
	Eigen::VectorXd g1(dof); 
	double th3_1 = -1.0*M_PI/2;
	for (int i = 0; i <= 100; i ++){
		robot->_q << 0.0, 0.5, th3_1;
		robot->updateModel();
		robot->gravityVector(g1);
		cout << th3_1 << " " << g1.transpose() << endl;
		th3_1 += step;
	}
	
	// G when th1=0, th3=0, 0<=d2<=2
	cout << " " << endl;
	cout << "printing g1, g2, g3 for 2fii)" << endl;
	Eigen::VectorXd g2(dof); 
	double d2_1 = 0.0;
	for (int i = 0; i <= 100; i ++){
		robot->_q << 0.0, d2_1, 0.0;
		robot->updateModel();
		robot->gravityVector(g2);
		cout << d2_1 << " " << g2.transpose() << endl;
		d2_1 += step1;
	}
