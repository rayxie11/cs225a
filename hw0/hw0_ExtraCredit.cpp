...
int main() {
	...
	*/
	robot->_q << 0, 0.6, M_PI/3, 0.6; // Joint 1,2,3 Coordinates (radians, meters, radians)
	...
	*/
	...
	// M when th1=0, d2=0.5, th3=pi/4, 0<=d4<=1
	cout << " " << endl;
	cout << "printing g1, g2, g3 for 2g)" << endl;
	Eigen::VectorXd g1(dof); 
	double step = 1.0/100;
	double d4 = 0.0;
	for (int i = 0; i <= 100; i ++){
		robot->_q << 0.0, 0.5, M_PI/4, d4;
		robot->updateModel();
		robot->gravityVector(g1);
		cout << d4 << " " << g1.transpose() << endl;
		d4 += step;
	}
